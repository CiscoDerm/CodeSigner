...
def main():
    ...
    try:
        if args.action == 'generate-keys':
            ...
            signer = CodeSigner()
            if args.password:
                signer.load_private_key("keys/private_key.pem", args.password)
            else:
                signer.load_private_key("keys/private_key.pem")
                
            signer.load_manifest_key("keys/manifest.key")
            ...
            if not args.public_key:
                ...
                sys.exit(1)
            success = verify_directory(args.directory, args.public_key, args.manifest_key)
